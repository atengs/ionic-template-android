        ...
        <ion-content :fullscreen="true">
            ...
            <div id="dashboard-container">
                ...
                <ion-card v-for="item in data[0] " :key="item.id">
                    <ion-card-header>
                        <ion-card-title style="color: black;">{{ weekday[new Date(item.absence_date).getDay()] + ', ' +
            new
...
export default defineComponent({
    ...
    async mounted() {
        this.getData();
        // console.log(this.$root.api_path)
        // console.log('mounted');
    },
    methods: {
        getData() {
            // hit api
            return axios
                .get(this.$root.api_path + "trx/absence/getByIdUsers/" + localStorage.getItem('id'), { dataType: "json" })
                .then((response) => {
