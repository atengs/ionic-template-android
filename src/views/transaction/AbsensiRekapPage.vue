        ...
        <ion-content :fullscreen="true">
            ...
            <div id="dashboard-container">
                ...
                <ion-card v-for="item in data[0] " :key="item.id">
                    <ion-card-header>
                        <ion-card-title style="color: black;">{{ weekday[new Date(item.attendee_date).getDay()] + ', ' +
            new
                Date(item.attendee_date).getDate() + ' ' + month[(new
                    Date(item.attendee_date).getMonth())] + ' ' + new Date(item.attendee_date).getFullYear()
                        ...
                        <ion-card-subtitle>
                            {{ (
            item.attendee_time_in == '00:00:00' ? (
                item.approval_1 == 0 ? 'Menunggu' :
                    (
                        item.approval_1 == 1 ? 'Disetujui' : (item.approval_1 == 2 ? 'Ditolak' : 'Libur Nasional')))
                : 'Masuk: ' + item.attendee_time_in + ' | ' + 'Pulang: ' +
                (item.attendee_time_out == null ? '' : item.attendee_time_out)) }}</ion-card-subtitle>
                    </ion-card-header>
...
export default defineComponent({
    ...
    methods: {
        getCutoff() {
            return axios
                .get(this.$root.api_path + "mst/masterCutoff/getByCustomerCode/" + localStorage.getItem('customer_code'), {
                    dataType: "json",
                })
                .then((response) => {
                    // binding data
        ...
        async getData() {
            ...
            // hit api
            return axios
                .post(this.$root.api_path + "trx/attendee/getRekapByPeriode", post)
                .then(async (response) => {
