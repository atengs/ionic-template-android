  ...
  <ion-page>
    <ion-content :fullscreen="true">
      <div class="container">
        ...
        <div class="header">
          <img src="../assets/img/logo_mtg.png" alt="logo_mtg">
        </div>
        <ion-card>
          <ion-card-header>
            <ion-card-title>HI, {{ name }}</ion-card-title>
            <ion-card-subtitle>
              <div class="jumbotron">
          ...
          <ion-card-content v-if="attendee_date != null">
            Absen {{ attendee }} Pukul {{ (attendee_time_out == null ?
      attendee_time_in : attendee_time_out)
          ...
          <ion-card-content v-if="absence_date != null">
            ...
            <ion-icon
              :icon="(approval_1 == 1 ? checkmarkCircleOutline : (approval_1 == 2 ? closeCircleOutline : timeOutline))">
            </ion-icon>
          </ion-card-content>
...
import { mail, logIn, logOut, mailOpen, location, document, personCircle, calendar, checkmarkCircleOutline, timeOutline, closeCircleOutline, notifications } from 'ionicons/icons';
...
export default {
  ...
  setup() {
    ...
    return { handleRefresh, useBackButton, router, printCurrentPosition, logIn, mail, mailOpen, logOut, location, document, personCircle, calendar, checkmarkCircleOutline, closeCircleOutline, timeOutline, notifications };
  },
  data() {
    ...
    const day_name = days[d.getDay()] + ', ' + d.getDate() + ' ' + month[(d.getMonth() + 1)] + ' ' + d.getFullYear();
    return {
      today: day_name,
      attendee: null,
  ...
  methods: {
    ...
    getData() {
      axios.get(this.$root.api_path + 'trx/attendee/getByUsersId/' + localStorage.getItem('id'))
        .then(async (r) => {
